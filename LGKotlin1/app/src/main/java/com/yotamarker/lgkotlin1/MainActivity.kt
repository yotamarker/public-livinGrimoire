...
class MainActivity : AppCompatActivity(), TextToSpeech.OnInitListener, AccelerometerListener{
    ...
    var chii = Chobit()
    var toggleTic = false
    var gyroX = 0.0f; var gyroY = 0.0f;var gyroCounter = 0;var gyroGate = true;var gyroDir = -1
    //var spoke = false
    ...
    override fun onAccelerationChanged(x:Float, y:Float, z:Float) {
        if((gyroX - x) > 2 && gyroGate){
            gyroCounter++
            gyroGate = false
            if(gyroX > 0){gyroDir = 1}else{gyroDir = -1}
        ...
        gyroX=x;gyroY=y;if((gyroDir > 0 && gyroX < 0)|| (gyroDir < 0 && gyroX > 0)){gyroGate = true}
    }
